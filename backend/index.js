...
- Bạn có **${subjects.length} môn học** với tổng cộng **${totalCredits} tín chỉ**.
- Điểm trung bình GPA: **${gpa4.toFixed(2)}/4.0** (Hệ 10: **${gpa10.toFixed(2)}**).

${subjects.length > 0 ? `📚 **Gợi ý học tập:**
- Điểm trung bình GPA hiện tại đang ở mức **${gpa4 >= 3.2 ? 'Giỏi/Xuất sắc 🌟' : gpa4 >= 2.5 ? 'Khá 👍' : 'Cần cải thiện nhiều ⚠️'}**.
- Hãy cố gắng duy trì hoặc nâng cao điểm số của các môn học hiện tại nhé!` : 'Bạn chưa thêm môn học nào. Hãy thêm môn học ở bảng điều khiển để tôi có thể phân tích điểm giúp bạn nhé!'}
...
- Tổng số tín chỉ tích lũy: ${totalCredits} tín chỉ.
- Điểm trung bình GPA:
  + Hệ 10: ${gpa10.toFixed(2)}/10.0
  + Hệ 4: ${gpa4.toFixed(2)}/4.0
- Danh sách môn học:
${subjects.map((sub, idx) => `  ${idx + 1}. Môn: ${sub.name} | Tín chỉ: ${sub.credit} | Điểm số: ${sub.score}`).join('\n') || '  (Chưa có môn học nào)'}
...
Nhiệm vụ của bạn:
1. Trả lời các câu hỏi liên quan đến lịch học, danh sách môn học và điểm số của sinh viên một cách chính xác dựa trên dữ liệu trên.
2. Đưa ra lời khuyên học tập cá nhân hóa, ví dụ: nhắc lịch học của ngày được hỏi, phân tích điểm số môn nào thấp để khuyên tập trung học hơn, hướng dẫn cách cải thiện GPA.
