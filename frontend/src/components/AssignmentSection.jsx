...
import { Calendar, Clock, CheckCircle2, Circle, AlertCircle, FileText, Plus, Edit2, Trash2, Check, X, Search, ArrowUpDown } from 'lucide-react';
...
export default function AssignmentSection({ 
  ...
  const handleEdit = (assign) => {
    ...
    // Format deadline from database (e.g. 2026-06-08T08:00:00.000Z) to datetime-local format (yyyy-MM-ddThh:mm)
    if (assign.deadline) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingAssignment) {
        ...
        addToast('Cập nhật bài tập thành công!', 'success');
        handleCancelEdit();
      } else {
        await axios.post(
        ...
        addToast('Thêm bài tập/deadline thành công!', 'success');
        setTitle('');
        setDescription('');
        setDeadline('');
        setSubjectId('');
      ...
      const msg = error.response?.data?.message || 'Không thể lưu bài tập.';
      addToast(msg, 'error');
    }
  };
  ...
  const handleDelete = async (id, name) => {
    ...
    try {
      ...
      addToast(`Đã xóa bài tập "${name}"!`, 'success');
      onAssignmentChange();
    } catch (error) {
      console.error('Assignment delete error:', error);
      const msg = error.response?.data?.message || 'Không thể xóa bài tập.';
      addToast(msg, 'error');
    }
  };
  ...
  const toggleStatus = async (assign) => {
    ...
    try {
      ...
      addToast('Lỗi khi cập nhật trạng thái bài tập.', 'error');
    }
  };

  // Helper to calculate countdown time in Vietnamese
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      ...
      <div className="glass-card">
        <h3 className="gradient-text" style={{ fontSize: '1.25rem', marginBottom: '1.25rem', display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          {editingAssignment ? 'Sửa Bài Tập / Hạn Nộp' : 'Thêm Bài Tập / Deadline'}
        </h3>

        <form onSubmit={handleSubmit}>
          <div className="form-group">
            <label className="form-label" htmlFor="assign-title">Tiêu đề công việc / Bài tập</label>
            <input 
              id="assign-title"
              type="text"
              className="form-input"
              placeholder="Ví dụ: Nộp báo cáo cuối kỳ, Làm bài tập tuần 5..."
              value={title}
          ...
          <div className="form-row">
            <div className="form-group">
              ...
              >
                <option value="">-- Khác / Không liên quan --</option>
                {subjects.map(sub => (
          ...
          <div className="form-group" style={{ marginBottom: '1.5rem' }}>
            ...
            <textarea 
              id="assign-desc"
              className="form-input"
              placeholder="Yêu cầu bài tập, tài liệu tham khảo..."
              rows="2"
              style={{ resize: 'vertical', minHeight: '60px', fontFamily: 'inherit', paddingTop: '8px' }}
              value={description}
          ...
          {filteredAssignments.length === 0 ? (
            <div className="empty-state">
              ...
              <p style={{ fontSize: '0.9rem' }}>
                {statusFilter === 'pending' && 'Tuyệt vời! Không có bài tập nào chưa hoàn thành.'}
                {statusFilter === 'completed' && 'Bạn chưa hoàn thành bài tập nào.'}
                {statusFilter === 'overdue' && 'Tuyệt vời! Không có bài tập nào quá hạn.'}
                {statusFilter === 'All' && 'Chưa có bài tập nào được thêm.'}
              </p>
            </div>
          ) : (
